...
foreach (var item in obj.GetAllChild())
{
  item.AddComponentIfnull<T>();
    ...
    public static List<UnityEngine.GameObject> GetAllChild( this UnityEngine.GameObject self ,bool includeInactive=false)   
    {
        List<UnityEngine.GameObject> Objs= new List<GameObject>();
 
        Transform[] allChildren = self.GetComponentsInChildren<Transform>(includeInactive);
