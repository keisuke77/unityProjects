...
public class follow : MonoBehaviour
{
    ...
    {
        _rb.linearVelocity = new Vector3(
            _rb.linearVelocity.x * 0.9f,
            _rb.linearVelocity.y,
            _rb.linearVelocity.z * 0.9f
        );
    ...
    void FallAnimation()
    {
        bool isFalling = _rb.linearVelocity.y < -4.0f;
        ...
        if (Physics.Raycast(pos, Vector3.down, out raycastHit))
        {
            ...
            if (distanceToGround > 1.0f && _rb.linearVelocity.y < -0.5f) { isFalling = true; }
        }
