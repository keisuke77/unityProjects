...
public List<controll> AddKey;
public List<controll> DownKey;

public List<controll> DecideKey;
public Color OutlineColor;

public AxisValue axisValueAdd;

public AxisValue axisValueDown;
...
if (axisValueAdd.Execute())
{
     num++;
     select();
}if (axisValueDown.Execute())
{
     num--;
     select();
}
        if (keiinput.Instance.GetKeys(AddKey))
        {
            num++;
            select();
        }
        if (keiinput.Instance.GetKeys(DecideKey))
        { CurrentSpriteEvent=spriteevents[num % spriteevents.Length];

            decide();
        }
        if (keiinput.Instance.GetKeys(DownKey))
        {
            num--;
            select();
        }
